...
function refresh_sh_config() {
    gem env
    export PATH=/usr/local/bin:/usr/bin:/bin:/usr/sbin:/sbin:/Library/Apple/usr/bin:/opt/homebrew/bin:$PATH
...
function clean_expired_archive_cache() {
    ...
    find $HOME/server/web/package/ios -mtime +20 -name "*.plist" -exec rm -rf {} \;
    find $HOME/server/web/package/ios -mtime +20 -name "*.ipa" -exec rm -rf {} \;
    echo "=============清理结束=============="
}
...
function gen_app_center_record() {
    # 获取version
    appVersion=$(xcodeproj show | grep "MARKETING_VERSION: " | grep -o '[0-9]\{1,\}.[0-9]\{1,\}.[0-9]\{1,\}' -m 1)
    # 获取build
    appBuild=$(xcodeproj show | grep "CURRENT_PROJECT_VERSION:" | grep -o '[0-9]\{1,\}' -m 1)

    $HOME/server/cmd/gen-package-json \
        workDir=="$HOME/server/web/app-center" \
        project==$workspace \
        platform==ios \
        env==$mode \
        branchId==$branchId \
...
function notify_tester() {
    ...
    if [ -z "${appName}" ]; then
        echo ""
         ...
         if [[ "${mode}" == "Release" ]]; then 
            # echo "通知钉钉机器人"
            # curl 'https://oapi.dingtalk.com/robot/send?access_token=34c43fab3a640a4307d40739643e912293611277f9f3f843b5e9d37e29f3d16f' -H  'Content-Type: application/json' -d  "{\"msgtype\": \"text\",\"text\": {\"content\": \"iOS: ${appName} 发布包构建成功,请访问http://test-pm3.kfang.com 进行下载\"}}"
            echo "通知飞书机器人"
            curl -X POST -H 'Content-Type: application/json' -d "{\"msg_type\":\"post\",\"content\":{\"post\":{\"zh_cn\":{\"title\":\"iOS: ${appName} ${appVersion} ${appBuild}发布包构建成功\",\"content\":[[{\"tag\":\"a\",\"text\":\"前往下载\",\"href\":\"https://test-pm3.kfang.com\"}]]}}}}" https://open.feishu.cn/open-apis/bot/v2/hook/cf11e8b1-8f8e-4c28-8160-832272229827
         fi        
    fi
}
...
echo "⑪⑪⑪ ⑪⑪⑪ ⑪⑪⑪ ⑪⑪⑪ ⑪⑪⑪"
make_dir_if_not_exist $HOME/server/web/package/ios
if [ -f "$target.ipa" ]; then
    cp $target.ipa $HOME/server/web/package/ios/$workspace-$branchId.ipa
elif [ -f "build/$target/$target.ipa" ]; then
    cp "build/$target/$target.ipa" $HOME/server/web/package/ios/$workspace-$branchId.ipa
else
    echo "ERROR: 未找到ipa包"
    exit 1
fi
cp $workspace-$branchId.plist $HOME/server/web/package/ios/$workspace-$branchId.plist
